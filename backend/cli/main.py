...
async def run_diagnosis(
    ...
    print("\n[1/3] 加载本体知识库...")
    service = DiagnosisService(ontology_path=ontology_path, model=model, load_ontology=load_ontology)
    ...
    try:
        if RICH_AVAILABLE:
            ...
            with console.status("[bold cyan]正在调用LLM进行智能诊断...", spinner="dots") as status:
...
async def interactive_mode(ontology_path: str, verbose: bool = False, load_ontology: bool = True, log_level: Optional[str] = None):
    """
...
def run_dtc_diagnosis(dtc_codes: List[str], role: str = "owner", verbose: bool = False, log_level: Optional[str] = None):
    ...
    for dtc in parsed:
        ...
        print(f"  {severity_icon} {dtc.code} | {dtc.category.value.upper():12} | {dtc.severity.value:8} | {dtc.description_zh}")
    ...
    print("\n" + "-" * 60)
    print("严重度分布:", " | ".join([f"{s.value}: {c}" for s, c in severity_counts.items()]))
...
3. 检查CAN通信链路""")
                ...
                print(f"【系统诊断】检测到故障码 {dtc_info.code}\n\n您好，车辆检测到故障码 {dtc_info.code}，建议到店进行专业检测。")
    
    print("\n" + "=" * 60)


def list_supported_dtcs(category: Optional[str] = None, verbose: bool = False, log_level: Optional[str] = None):
...
"""
    ...
    if args.interactive:
        asyncio.run(interactive_mode(ontology_path, args.verbose, load_ontology, args.log_level))
    elif args.symptom:
        if role is None:
